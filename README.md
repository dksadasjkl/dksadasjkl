<!--
**dksadasjkl/dksadasjkl** is a ✨ _special_ ✨ repository because its `README.md` appears on your GitHub profile.
-->

<!-- 타이틀 -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=green&height=300&section=header&text=Seo%20Chang%20Hyeon&fontSize=70&fontColor=FFFFFF"/>
</p>

---

## 👋 Introduction
안녕하세요, **백엔드 중심의 풀스택 개발자 서창현**입니다.  
서비스의 안정적인 운영을 위해 **설계·개발·배포·운영 전 과정을 고려하는 개발자**를 지향합니다.  
방문해 주셔서 감사합니다.

---

## 🌟 About Me

<details>
  <summary><strong>나의 일하는 방식 (성장과 책임감)</strong></summary>
  맡은 기능을 끝까지 완성하는 책임감을 가장 중요하게 생각합니다.  
  단순히 동작하는 코드에 그치지 않고, <b>유지보수성과 확장성을 고려한 구조</b>를 고민하며 개발합니다.  
  문제를 해결하는 과정을 성장의 기회로 삼는 개발자입니다.
</details>

<details>
  <summary><strong>나의 소통 방식 (사람과 팀워크)</strong></summary>
  팀원의 의견을 존중하며, 기술적인 판단에는 항상 근거를 바탕으로 소통하려 노력합니다.  
  코드 리뷰와 협업 과정에서 명확한 설명과 합리적인 조율을 통해  
  팀 전체의 개발 효율과 코드 품질을 함께 높이는 역할을 지향합니다.
</details>

<details>
  <summary><strong>학습과 사고 방식 (이해 중심)</strong></summary>
  빠른 구현보다 <b>왜 이렇게 설계해야 하는지</b>를 이해하는 것을 중요하게 생각합니다.  
  공식 문서와 레퍼런스를 기반으로 문제를 분석하고,  
  개인의 성장을 넘어 팀과 함께 성장하는 개발 문화를 지향합니다.
</details>

---

## 🎓 Education
- **빅데이터·AI 기반 시니어 디지털 헬스케어 웰니스 플랫폼 개발**  
  (국비 교육 과정)  
  2023.12.28 ~ 2024.05.17

---

## ⚒️ Tech Stack

### 🖥️ Frontend
<img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=React&logoColor=black"/><img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white"/><img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white"/><img src="https://img.shields.io/badge/Flutter-02569B?style=flat-square&logo=flutter&logoColor=white"/>

### ⚙️ Backend
<img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=openjdk&logoColor=white"/><img src="https://img.shields.io/badge/Spring Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white"/><img src="https://img.shields.io/badge/Spring Security-6DB33F?style=flat-square&logo=springsecurity&logoColor=white"/>

### 🗄️ Database
<img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white"/><img src="https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=white"/>

### ☁️ DevOps / Infra
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white"/><img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white"/>

### 🧩 Others
<img src="https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white"/><img src="https://img.shields.io/badge/MyBatis-4479A1?style=flat-square&logo=databricks&logoColor=white"/>

---

📌 **지향하는 개발자**
- 서비스 전체 흐름을 이해하고 설계할 수 있는 백엔드 개발자  
- 협업과 운영을 고려한 실무 중심의 개발자  
- 안정성과 확장성을 함께 고민하며 성장하는 개발자

---

## 📫 Contact

<p align="center">
  <a href="https://code-rjqnrdl.tistory.com/" target="_blank">
    <img src="https://img.shields.io/badge/Tistory-FE5F50?style=flat-square&logo=Tistory&logoColor=white" height="35"/>
  </a>
  &nbsp;
  <a href="mailto:ckdgus6038@gmail.com">
    <img src="https://img.shields.io/badge/Email-ckdgus6038@gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white" height="35"/>
  </a>
</p>

---

<div align="center">
  <table>
    <tr>
      <td>
        <a href="https://github.com/dksadasjkl">
          <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=dksadasjkl&layout=compact" alt="Top Languages">
        </a>
      </td>
      <td>
        <img src="https://github-readme-stats.vercel.app/api?username=dksadasjkl&theme=swift&show_icons=true" alt="GitHub Stats">
      </td>
    </tr>
  </table>
</div>

---
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=green&height=200&section=footer"/>
</p>
